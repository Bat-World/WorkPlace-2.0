...
function getPriorityProps(priority: Task["priority"]) {
  switch (priority) {
    case "high":
      return {
        label: "Яаралтай",
        className: "bg-[#450B2B] border border-[#5A2241] text-[#FCC3EC]",
      };
    case "medium":
      return {
        label: "Чухал",
        className: "bg-[#201302] border border-[#372B15] text-[#F9D769]",
      };
    case "low":
    default:
      return {
        label: "Энгийн",
        className: "bg-[#191C45] border border-[#32316B] text-[#B5BFF3]",
      };
  }
}
...
const SortableItem = React.memo(
  ({
    ...
    const { label: priorityLabel, className: priorityClass } = useMemo(
      () => getPriorityProps(task.priority),
      [task.priority]
    );
          ...
          >
            <div className="w-full flex mb-3 items-center justify-between">
              <div className="flex gap-2">
                <span
                  className={`text-xs rounded-full px-3 py-1.5 ${priorityClass}`}
                >
                  {priorityLabel}
                </span>
                {task.label && (
            ...
            <div className="text-sm font-light text-[var(--background)]/50 mb-1">
              {task.description}
            </div>
