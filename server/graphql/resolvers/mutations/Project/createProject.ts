...
export const createProject = async (
  _: any,
  args: { input: { title: string; description?: string; invitees?: string[]; labels?: string[]; userId?: string } }
,
  context: any
) => {
  const { userId: contextUserId, prisma } = context;
  const { title, description, invitees = [], labels = [], userId: inputUserId } = args.input;
